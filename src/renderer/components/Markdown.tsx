...
function renderNode(n: MdNode, key: number, mentions: ResolvedMention[], jumbo: boolean): ReactNode {
  switch (n.type) {
    ...
    case 'mention_user': {
      ...
      return <span key={key} className="bg-[#282b50] text-[#a9bbff] font-medium rounded px-1 hover:bg-[#5865f2] hover:text-[#fbfcff] cursor-pointer">@{m?.name ?? n.id}</span>;
    }
    case 'mention_channel': {
      ...
      return (
        <span
          key={key}
          className="bg-[#282b50] text-[#a9bbff] font-medium rounded px-1 hover:bg-[#5865f2] hover:text-[#fbfcff] cursor-pointer"
          onClick={(e) => {
            e.preventDefault();
    ...
    case 'mention_broadcast':
      return <span key={key} className="bg-[#282b50] text-[#a9bbff] font-medium rounded px-1 hover:bg-[#5865f2] hover:text-[#fbfcff] cursor-pointer">@{n.name}</span>;
    case 'custom_emoji':
...
function DiscordLinkChip({ url, link }: { url: string; link: { guildId: string; channelId: string; messageId?: string } }) {
  ...
  return (
    <span
      ...
      className="inline-flex items-center gap-0.5 align-baseline bg-[#282b50] text-[#a9bbff] font-medium rounded px-1 hover:bg-[#5865f2] hover:text-[#fbfcff] cursor-pointer"
    >
      <span>#{name ?? '…'}</span>
      {link.messageId && (
        <>
